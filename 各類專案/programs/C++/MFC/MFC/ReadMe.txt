...
應用程式精靈已經為您建立了這個 MFC 應用程式。這個應用程式
不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。

這個檔案的內容摘要，包含各個構成 MFC 應用程式的檔案。
...
MFC.cpp
    這是主要的應用程式原始程式檔，它包含應用程式類別 CMFCApp。
...
MFCDoc.h、MFCDoc.cpp - 文件
    這些檔案包含您的 CMFCDoc 類別。編輯這些檔案，即可新增您的特
    殊文件資料，並實作檔案儲存和載入 (透過 CMFCDoc::Serialize)。

MFCView.h、MFCView.cpp - 文件的檢視
    這些檔案包含您的 CMFCView 類別。
    CMFCView 物件是用來檢視 CMFCDoc 物件。

res\MFCDoc.ico
    這個圖示檔是用來做為 CMFCDoc 類別之 MDI 子視窗的圖示。
    這個圖示是包含在主要資源檔 MFC.rc 內。
...
StdAfx.h、StdAfx.cpp
    這些檔案是用來建置名為 MFC.pch 的先行編譯標頭 (PCH) 檔，
