...
namespace DDoS_get
{
    ...
    //    {
    //        TcpListener l = new TcpListener(IPAddress.Parse("192.168.1.5"), 80); l.Start();
    //        Socket s = l.AcceptSocket();
    //        do
    //        {
    //            byte[] bfr = new byte[500];
    //            s.Receive(bfr);
    //            write(Encoding.ASCII.GetString(bfr));
    //            System.IO.StreamWriter sw = new System.IO.StreamWriter("d:/output/http.txt");
    //            sw.Write  (Encoding.ASCII.GetString(bfr));
    //            sw.Flush ();
    //            for (int ii = 0; ii != 100000000; ii++)
    //            {
